     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  ...
  // Ricardo: number of classes
  int K;
  for(i=0;i<totdoc;i++) {
    K = rankvalue[i] > K ? rankvalue[i] : K;
  }
  int *ks = calloc(K, sizeof(int));
  for(i=0;i<totdoc;i++) {
    ks[(int)rankvalue[i]]++;
  }
  ...
  for(i=0;i<totdoc;i++) {
	...
	cost=1;
        ...
        // Ricardo: minha mudança para pesos serem balance para as várias diferenças
        double comb_n = 2*ks[(int)rankvalue[i]]*ks[(int)rankvalue[j]];
        double comb_k = (K*(K-1))/2.;
        cost = totpair / (comb_k * comb_n);
